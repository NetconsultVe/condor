...
class cne_controler extends Controller
{
    ...
    public function GetParroquias($id){
        $sql_string = "SELECT cne_geopq.NameParroauia,cne_geopq.CodParroquia,cne_geopq.IdParroquia FROM cne_geopq WHERE cne_geopq.IdMunicipio = " . $id;
        $resp = DB::select($sql_string);
        return response()->json($resp);
    }
